...
class NormedLinear(nn.Module):
    r"""
        linear layer with normalized features and weight vectors
        (meant to be used with a rescaling parameter like sigma for cosine layers but not trained during training)
    """
...
class CosineLinear(nn.Module):
    r"""
    Cosine linear layer with multiple proxy weight vectors support
    set nb_proxies to 1 for the true cosine linear layer
    """
...
class biFeatCosineLinear(nn.Module):
    r"""
    Cosine linear layer with multiple proxy weight vectors support
    set nb_proxies to 1 for the true cosine linear layer
    """
    def __init__(self, in_features, out_features, sigma=True):
        super(biFeatCosineLinear, self).__init__()
        self.in_features = in_features
        self.out_features = out_features
        self.weight = nn.Parameter(torch.Tensor(self.out_features, in_features))
        if sigma:
    ...
    def forward(self, input1, input2):
        x = torch.cat([F.normalize(input1, dim=1), F.normalize(input2, dim=1)], dim=1)
        out = F.linear(x, F.normalize(self.weight, p=2, dim=1))
