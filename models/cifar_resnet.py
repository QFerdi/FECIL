...
class CifarResNet(nn.Module):
    ...
    def forward(self, x, mixup_data=None, x2_mix=None, layer_mix=-1):
        bsz = x.size(0)
        if (layer_mix >= 0 and x2_mix is not None):
            x = torch.cat((x, x2_mix), 0)
        if (layer_mix == 0):
            x = mixup_data.mixup_manifold(x, bsz)
        x = self.conv_1_3x3(x)  # [bs, 16, 32, 32]
        x = F.relu(self.bn_1(x), inplace=True)
        f0 = x
        if (layer_mix == 1):
            x = mixup_data.mixup_manifold(x, bsz)
        x = self.stage_1(x)  # [bs, 16, 32, 32]
        f1 = x
        if (layer_mix == 2):
            x = mixup_data.mixup_manifold(x, bsz)
        x = self.stage_2(x)  # [bs, 32, 16, 16]
        f2 = x
        if (layer_mix == 3):
            x = mixup_data.mixup_manifold(x, bsz)
        x = self.stage_3(x)  # [bs, 64, 8, 8]
        f3 = x
        if(layer_mix == 4):
            x = mixup_data.mixup_manifold(x, bsz)
