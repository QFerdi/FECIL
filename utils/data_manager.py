...
class DataManager(object):
	def __init__(self, dataset_name, shuffle, seed, init_cls, cls_per_increment):
		...
		while sum(self._increments) + cls_per_increment < len(self._class_order):
			self._increments.append(cls_per_increment)
	...
	def get_dataset(self, indices, source, mode, max_data_per_class=None, appendent=None, ret_data=False):
		...
		for idx in indices:
			...
			if(max_data_per_class is not None):
				class_data = class_data[:max_data_per_class,...]
				class_targets = class_targets[:max_data_per_class]
	...
	def _setup_data(self, dataset_name, shuffle, seed):
		...
		# Order
		nb_classes = len(np.unique(self._train_targets))
		order = [i for i in range(nb_classes)]
		if shuffle:
			np.random.seed(seed)
		...
		if(hasattr(idata, "classes")):
			self._classes_names = np.array(idata.classes)
		else:
			self._classes_names = np.array([i for i in range(nb_classes)])
	def _select(self, x, y, low_range, high_range):
		idxes = np.where(np.logical_and(y >= low_range, y < high_range))[0]
		return x[idxes], y[idxes]

	def add_train_trsf(self, trsf):
		self._train_trsf.append(trsf)
